...
- **Geometria RHMI G30 (z `gmap_nav.log`, NIE zgadywać): `rhmi=1440x540 app=748x473 visible=838x540 padding=90,67`.** Pozycje od rogu obszaru treści. PION: Y=0 = linia belki BMW (potwierdzone build 89). POZIOM: karta CELOWO podchodzi pod lewy pas paddingu (tam auto rysuje strzałkę „<" NA panelu): PNG (−paddingLeft, 0) 223×(appHeight−8), mapa (−paddingLeft+223, 0) (visibleWidth−223−8)×(appHeight−8). Marginesy karty w ROZMIARZE komponentów, maska tylko zaokrągla prawe rogi. Wymiary logowane przy każdym połączeniu (X1 może mieć inne).
